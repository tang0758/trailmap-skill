...
每条路径都会记录它创建时的来源上下文，包括为什么创建、当时已确认什么、有哪些假设、有哪些约束。这用于 `mark resume ... clean` 时避免把其他路径的结论污染进来。
...
如果当前没有活跃主题，`mark` 会创建新主题和根路径。根路径的 `parent` 是空。

如果当前已经有 active path，`mark` 表示“当前路径下面出现了子分叉”：
...
```text
mark A 里面现在可能是缓存问题，也可能是刷新逻辑问题，先查刷新逻辑
```
...
`mark list` 是跨主题工作台总览。它显示每个主题下所有状态的路径：
...
`mark show <key>` 只在 active topic 内查找路径。第一版不支持 `mark show <topic_id>`。
...
```text
mark close A done
mark close B blocked
mark close C discarded
```

关闭后路径状态是 `closed`，关闭分类写入 `closed_as`。如果关闭的是当前 active path，`topic.active` 会变成空；之后用 `mark resume` 激活另一条路径。
...
- `mark list` 是跨主题工作台，用来看所有主题和路径状态。
- `mark map` 是当前主题结构图，用来看路径树长什么样。
...
如果某条路径记录了代码改动，`mark resume ... clean` 时会提醒你：clean 只隔离对话上下文，不隔离工作区代码。

## 推荐工作流

1. 遇到多个根方向时，用 `mark` 创建主题和根路径。
2. 当前路径内部又分出子方向时，用 `mark` 创建 child paths。
3. 工作中临时想到旁边的可能性，用 `mark pending <idea>` 记录 sibling pending path。
4. 每完成一次关键尝试，用 `mark update <key>` 记录阶段结果。
5. 需要回到另一条路径时，用 `mark resume <key> clean`。
6. 如果需要参考其他路径结论，用 `mark resume <key> informed`。
7. 路径结束后，用 `mark close <key> done|blocked|discarded` 关闭。
...
```text
mark pending 可能是服务端限流导致，先记下来，不切换当前路径
```
