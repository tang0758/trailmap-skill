...
它解决的问题是：当 AI 给出多个方向，或者你自己尝试了多条路径后，不再依赖记忆去回想“还有哪些路没走”“A 路径试过什么”“现在要不要回到 B”。或者再尝试某些路径时候遗留了之前想尝试的路径。有了这个 skill，会以 map 的方式记录你尝试的路径、想要尝试的路径、路径结果等信息。
...
如果当前 Claude Code 会话启动时还不存在顶层 skills 目录，安装后建议重启会话。
