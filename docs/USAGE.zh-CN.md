...
- 使用与当前 active path 相同的 `parent`，因此是 sibling path
- 初始状态设为 `pending`
- 不生成当前路径的离开摘要
- 不改变当前路径的 `active` 状态
- 完全保持 `topic.active` 不变
...
`map` 默认输出当前主题的 Mermaid `mindmap`。它根据 `paths[].parent` 推导树，展示 key、标题、状态和 closed 路径的 `closed_as`，不展示完整 updates。
