...
如果用户表达暗示要关闭、判定无效、完成或阻塞某条路径，但没有提供 `closed_as`，Trailmap 必须要求用户选择 `done`、`blocked` 或 `discarded`。在关闭分类被明确确认前，不写入 closed update。
...
`subagent <key>` 为活跃主题中的已有路径启动 subagent 探索。目标路径必须存在，不能是当前主会话 active path，不能是 closed，且不能已经有 `agent_run.status: running` 或 `agent_run.status: reported`。reported run 必须先被接受、拒绝、完成、取消或以其他方式处理后，才能启动新的 run。
...
如果 branch 创建、目录创建、Git worktree 创建或 subagent 启动失败，Trailmap 会把该路径记录为 `agent_run.status: blocked` 和 `agent_run.worktree.status: failed`。它不会启动 subagent，也不会 fall back 到 shared workspace。下一步应修复底层问题后用 `--worktree` 重试，或显式另行启动不带 `--worktree` 的 shared-workspace run。
...
对于 `pending <idea> --subagent --worktree`，确认草案还必须包含 worktree startup details：context mode、base ref 和 sha、branch、worktree path、主工作区 dirty 状态、`.gitignore` 更新和安全提示。

对于多 path `--subagent B,C --worktree`，每条被选中的 path 都独立创建和记录。Single-path failure does not roll back successful paths；成功路径保持 `running` 和 `ready`，失败路径记录 `blocked` 和 `failed`。
...
缺少必填字段的 report 不能转换成普通 `update <key>` 草案。Trailmap 应要求 subagent 或用户补齐缺失字段，或在确认后用简短原因将 run 标记为 `blocked` 或 `failed`。
...
- revert 文件
- commit 代码
- 创建 Git 分支，除非用户明确确认了 `--worktree` 草案
- 切换主工作区 branch
- 回滚另一条路径产生的工作
